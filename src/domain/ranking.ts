import type { ListFavoriteState } from './types';
...
const score = (entry: ListFavoriteState, now: number): number => {
  const ageMs = now - entry.lastUsedAt;
  return entry.usageCount * Math.exp((-ageMs * Math.LN2) / HALF_LIFE_MS);
};

const sortByScoreDesc = (a: ListFavoriteState, b: ListFavoriteState, now: number): number => {
...
/**
 * Rank per-list favorite state into the favorites shelf.
 *
 * Rules:
 ...
 */
export const rankListFavorites = (
  entries: readonly ListFavoriteState[],
  now: number,
): ListFavoriteState[] => {
